...
class BlockManager:
    ...
    def allocate(self, seq: Sequence):
        ...
        """
        # NOTE: 为 seq 分配倒置页表，倒置页表的规模为 num_blocks
        assert not seq.inverted_page_table, "seq.inverted_page_table must be empty"
        seq.inverted_page_table = [-1] * self.num_blocks

        assert not seq.block_table, "seq.block_table must be empty"
        h = -1
        cache_miss = False
        for i in range(seq.num_blocks):
            token_ids = seq.block(i)
            # NOTE: 仅当 seq 序列的 Block_i 满时计算其 hash ID 
            h = self.compute_hash(token_ids, h) if len(token_ids) == self.block_size else -1
            # NOTE: 从 hash 表中，根据 hash(seq.block(i)) 查找是否存在可以复用的 Block 
            block_id = self.hash_to_block_id.get(h, -1)
            if block_id == -1 or self.blocks[block_id].token_ids != token_ids:
                cache_miss = True
            # NOTE: prefix caching，如果 cache miss，则分配新的 Block；否则尝试复用复用
            if cache_miss:
                block_id = self.free_block_ids[0]
                block = self._allocate_block(block_id)
            else:
                # NOTE: 如果可以复用 Block，则更新 seq 的 cached_tokens 计数，并增加 Block 的引用计数
                seq.num_cached_tokens += self.block_size
                if block_id in self.used_block_ids:
                    block = self.blocks[block_id]
                    block.ref_count += 1
                else:
                    # NOTE: 明明是可以复用的情况，为什么还需要重新分配 Block？Block 的引用计数为 0，所以需要重新分配 Block，并更新 Block 的引用计数
                    block = self._allocate_block(block_id)
            if h != -1:
            ...
            # NOTE: 增加倒置页表
            seq.inverted_page_table[block_id] = i
